...
document.querySelector('.tlacitko-hod').addEventListener('click', function () {
    if (hraProbiha) {
        ...
        if (kostka !== 1) {
            ...
            /* Lze zapsat pomocí if-else bloku
            if (aktivniHrac === 0) {
                aktivniHrac = 1;
            } else {
                aktivniHrac = 0;
            ...
            */
            ...
            // Nebo jako ternární operátor (podmínka ? příkaz při splnění podmínky : příkaz při nesplnění podmínky)
            // viz funkce dalsiHrac()
            dalsiHrac();
        }
    }
});
...
document.querySelector(".tlacitko-dost").addEventListener('click', function () {
    if (hraProbiha) {
        ...
        // Aktualizovat uživ. prostředí (zobrazení celkových bodů)
